...
import { listMyInvoices } from "../../services/tenantPortalService";
...
export default function TenantBills() {
  ...
  return (
    <div className="fade-in">
      {toast && (
        <div className="paid-toast" data-testid="bill-toast">
          <CheckCircle2 size={18} /> {toast}
        </div>
      )}

      {payingInv && <QrisPayModal invoice={payingInv} onClose={closeQris} />}

      <div className="page-head">
        <div>
          <h1 style={{ fontSize: 28 }}>Tagihan Saya</h1>
          <div className="sub">
            {rows.length} invoice total · {current.length} belum lunas
          </div>
        </div>
      </div>
                    ...
                    {i.days_diff != null && (
                      <>
                        ...
                        {i.days_diff === 0
                          ? "hari ini"
                          : i.days_diff > 0
                            ? `${i.days_diff} hari lagi`
                            : `terlambat ${Math.abs(i.days_diff)} hari`}
                      </>
                    )}
                  </div>
                </div>
